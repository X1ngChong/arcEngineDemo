 ...
 * Near:{distance:距离的远近,location:方位}
 * @author JXS
 */
...
//        String[] textNames = {"Group"};
                            ...
                            for (Integer childId2 : childIds) {
                                if (!Objects.equals(childId1, childId2)) {
                                    ...
                                    while (result3.hasNext()) {
                                        ...
                                        // 添加 NEAR 关系
                                        addNearRelation(session2, childId1, childId2, location, distance);
                                    }
                                }
                            }
                        }
    ...
    public static void addNearRelation(Session session, Integer nodeId1, Integer nodeId2, String location, String distance) {
        // 检查是否已经存在 NEAR 关系
        ...
        if (!checkResult.hasNext()) {
            ...
            session.run("MATCH (n1), (n2) " +
                    "WHERE ID(n1) = $id1 AND ID(n2) = $id2 " +
                    "CREATE (n1)-[:NEAR {location : $location, distance: $distance}]->(n2) ",
                    parameters("id1", nodeId1, "id2", nodeId2, "location", location, "distance", distance));
